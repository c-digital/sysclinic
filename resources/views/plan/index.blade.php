...
{{--                                            <a href="#" class="btn btn-primary btn-icon m-1">{{__('Free')}}</a>--}}
                                    ...
                                    @if($plan->id != 1 && $plan->id != \Auth::user()->plan)
                                        @if(\Auth::user()->requested_plan != $plan->id)
                                            <a data-bs-toggle="modal" data-bs-target="#exampleModal_{{$plan->id}}" href="#" class="btn btn-primary btn-icon m-1" data-title="{{__('Send Request')}}" data-bs-toggle="tooltip" title="{{__('Send Request')}}">
                                                Pago en Bolivia
                                            </a>
                                        @else
                                            @if(\Auth::user()->type != 'super admin')
            ...
            <div class="modal fade" id="exampleModal_{{$plan->id}}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
              <div class="modal-dialog">
                <div class="modal-content">
                    <form enctype="multipart/form-data" method="POST" action="{{ route('send.request') }}">
